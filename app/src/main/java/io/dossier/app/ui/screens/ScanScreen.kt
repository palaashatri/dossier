...
fun ScanScreen(onScanComplete: () -> Unit) {
    ...
    LaunchedEffect(isScanning) {
        if (isScanning) {
            ...
            liveLogs.add("Scan complete. Preparing report...")
            // Short delay to allow UI to update the log message before transition
            delay(300)
            // Call onScanComplete to navigate to report screen
            onScanComplete()
        }
    }
