...
Not complete: true suspended pause/resume, sole coordinator ownership, universal in-flight payload coverage across every stage, a general coordinator-owned parser/frontier loop beyond the persisted plan commitment, migration of remaining custom resolver work to provider lifecycle events and full-stage recovery beyond the encrypted result path. Breach, deterministic post-processing, graph-stage and relationship-confidence slices now have bounded encrypted reuse, but the worker still reruns earlier scan stages before reaching those boundaries. External force-stop/relaunch and emulator-reboot restoration of the result path now pass with a uiTest-only synthetic fixture.

The encrypted resume record stores the seed input, selected mode, deep-scan flag, per-scan strong-face-policy flag, the selected plan fingerprint plus bounded provider IDs for new requests, an allow-listed mode/stage-order plan summary, an allow-listed semantic stage ledger bound to the exact worker owner, bounded per-stage output counts, bounded breach-stage summaries, bounded post-processing analysis checkpoints, bounded graph-stage and relationship-confidence checkpoints and public search/image payload summaries. New WorkManager rows receive only opaque request/generation UUID references. Prepared/current generations, plan metadata, stage metadata/output counts, breach/post-processing/graph/confidence summaries and public search/image envelopes are still request checkpoints rather than a general parser/frontier state or universal in-flight stage payloads. The external smoke check validates only durable result restoration through the production UI with a synthetic fixture; it does not establish full-stage process-death/reboot recovery or physical-device acceptance.
...
Debug JVM unit tests          PASS — 787 tests / 130 result XML files / 0 failures / 0 errors / 0 skips
uiTest JVM unit tests         PASS — 787 tests / 130 result XML files / 0 failures / 0 errors / 0 skips
Android-test Kotlin compile  PASS — `compileUiTestAndroidTestKotlin`
Debug APK                     PASS — 115,597,800 bytes / SHA-256 D3833B06644635376A92641E1F788DA97B5D7DCF58DAF5B8A006EE1D5877735C
uiTest APK                    PASS — 243,112,377 bytes / SHA-256 DF1AFD51C6F5EDD096E48FCDB3B4252FE88A11BC7A203BEF2A38867E59AB1D57
