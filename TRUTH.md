...
Debug JVM unit tests          PASS — 834 tests / 137 result XML files / 0 failures / 0 errors / 0 skips
uiTest JVM unit tests         PASS — 834 tests / 137 result XML files / 0 failures / 0 errors / 0 skips
Android-test Kotlin compile  PASS — `compileUiTestAndroidTestKotlin`
Debug APK                     PASS — 119,488,689 bytes / SHA-256 470848FBC0FECB0BABE79B2FC0869236C4D639BDB694E4D2A00371274326D9BA
uiTest APK                    PASS — 243,770,885 bytes / SHA-256 9203946510080202682F65B0E42579DA94D7376340E83209E58F921CDE836ED2
...
Graph relation normalization PASS — 12 EntityGraphBuilder + 6 GraphSchemaV2 + 11 GraphEvidenceReconciliation + 3 CaseEvidenceIdMigration JVM tests; case/whitespace variants merge without dropping provenance and entity evidence IDs are trimmed/migrated/deduplicated/bounded
Recovery diagnostics            PASS — 5 ProfileInitialPassExecutor + 15 ScanCoordinatorRuntime JVM tests; exact direct-profile checkpoint reuse/rerun counts are sanitized, stage-allowlisted, count-bounded and rejected after terminal state
Visual QA                     NOT RUN in this validation session
```

The latest focused additions keep the pinned executable-rule health report separate from the 78 authored-provider report, publish sanitized direct-profile checkpoint reuse/rerun counts through the coordinator snapshot, normalize graph entity and relation evidence IDs by trim/migration/deduplication with a 256-ID bound, normalize graph relation keys by trim/case-insensitive comparison without rewriting the first persisted relation spelling or dropping evidence IDs, persist a bounded face-consistency result only across a stable model commitment, and confine CaseStore deletion paths to safe case IDs and the canonical case directory.
