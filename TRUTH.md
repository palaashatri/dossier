...
- **Last validated implementation commit:** `84f07d8` (encrypted deterministic graph-stage checkpoint; includes `bc623f0` WhatsMyName conversion-parity maintenance validation and the prior `7fd49df`, `67742db`, `0cbc2ca`, `c404592`, `acc325a`, `1764233`, `9bc1cd3`, `a810d0c`, and `4ee7e14` tranches)
- **Last validation date:** **2026-08-25**
- **Current-session validation of the implementation plus the preserved pre-existing uiTest fixture edit:** provider and pinned-catalog audits passed; the provider-maintenance/audit fixture suite passed 11/11 and reports the pinned WhatsMyName hashes, 716 source records, 644 executable rules, 644/644 runtime-equivalent conversions, 0 conversion errors, 0 primary-catalog ID collisions and 72 policy exclusions without changing the 78-provider count; both debug and uiTest JVM suites passed 775 tests across 128 result XML files each; debug/uiTest APKs assembled; Android-test Kotlin compiled; lint completed with zero errors; and the complete connected uiTest suite passed 53/53 on the API 36 `medium_phone` emulator, including the provider-health panel, 8 WorkManager pause/resume tests, encrypted Activity-recreation recovery, bounded breach-stage checkpoint behavior, bounded post-processing and graph-stage checkpoint/accessibility semantics, live finding/profile/media correction semantics, focused breach/report/reverse-media accessibility semantics, exact evidence-key fail-closed behavior, bounded CaseStore save behavior, graph/export redaction semantics, report-evidence semantics, async saved-case persistence, HUD status semantics and reverse-video picker semantics. A fresh external ADB `am force-stop`/relaunch and emulator-reboot smoke check on the current artifact restored the encrypted background result and confirmed the production `MainActivity` resumed after both launches. The dangling-evidence audit, source-page-aware reverse-image candidate deduplication, graph checkpoint codec/store tests and WhatsMyName conversion parity are JVM/no-network tested; post-processing and graph-stage checkpoint reuse remain code-tested for exact request/plan/owner, input-digest, size/shape, record-size fallback and TTL binding but were not independently process-kill validated on the emulator. Physical-device, broad accessibility and full-stage recovery gates remain open.
...
Not complete: true suspended pause/resume, sole coordinator ownership, universal in-flight payload coverage across every stage, a general coordinator-owned parser/frontier loop beyond the persisted plan commitment, migration of remaining custom resolver work to provider lifecycle events and full-stage recovery beyond the encrypted result path. Breach, deterministic post-processing and deterministic graph-stage slices now have bounded encrypted reuse, but the worker still reruns earlier scan stages before reaching those boundaries. External force-stop/relaunch and emulator-reboot restoration of the result path now pass with a uiTest-only synthetic fixture.

The encrypted resume record stores the seed input, selected mode, deep-scan flag, per-scan strong-face-policy flag, the selected plan fingerprint plus bounded provider IDs for new requests, an allow-listed mode/stage-order plan summary, an allow-listed semantic stage ledger bound to the exact worker owner, bounded per-stage output counts, bounded breach-stage summaries, bounded post-processing analysis checkpoints, bounded graph-stage checkpoints and public search/image payload summaries. New WorkManager rows receive only opaque request/generation UUID references. Prepared/current generations, plan metadata, stage metadata/output counts, breach/post-processing/graph summaries and public search/image envelopes are still request checkpoints rather than a general parser/frontier state or universal in-flight stage payloads. The external smoke check validates only durable result restoration through the production UI with a synthetic fixture; it does not establish full-stage process-death/reboot recovery or physical-device acceptance.
...
Debug JVM unit tests          PASS — 775 tests / 128 result XML files / 0 failures / 0 errors / 0 skips
uiTest JVM unit tests         PASS — 775 tests / 128 result XML files / 0 failures / 0 errors / 0 skips
Android-test Kotlin compile  PASS — `compileUiTestAndroidTestKotlin`
Debug APK                     PASS — 115,581,416 bytes / SHA-256 9056DD9ED30BADF4F269656B7E0579218B98646101C2C81254AF0B6EFD6CE22C
uiTest APK                    PASS — 243,095,993 bytes / SHA-256 AF6AB1059AF573386B28ACECE5C161115DFBC763620C9DA3282B18D0A15967A4
