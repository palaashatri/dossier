...
- **Last validated implementation commit:** `ab74b37` (sanitized deterministic later-stage recovery diagnostics, coordinated-scan HUD wiring, and inspectable bounded graph-entity provenance truncation diagnostics, on top of the prior face-checkpoint/CaseStore hardening)
- **Last validation date:** **2026-08-25**
- **Current-session validation of the implementation plus the preserved pre-existing uiTest fixture edit:** provider and pinned-catalog audits passed; the provider-maintenance/audit fixture suite passed 11/11 and reports the pinned WhatsMyName hashes, 716 source records, 644 executable rules, 644/644 runtime-equivalent conversions, 0 conversion errors, 0 primary-catalog ID collisions and 72 policy exclusions without changing the 78-provider count; both debug and uiTest JVM suites passed 839 tests across 137 result XML files each; debug/uiTest APKs assembled; Android-test Kotlin compiled; lint completed with zero errors; and the complete connected uiTest suite passed 53/53 on the API 36 `medium_phone` emulator, including separate authored/WhatsMyName provider-health diagnostics, direct-profile and deterministic later-stage recovery-diagnostics projection regressions, graph-entity-provenance regressions including deterministic truncation diagnostics, the relation-key graph regression, face-checkpoint codec/store/model-race regressions, CaseStore path/deletion regressions, 8 WorkManager pause/resume tests, encrypted Activity-recreation recovery, bounded breach-stage behavior, bounded post-processing and graph-stage checkpoint/accessibility semantics, live finding/profile/media correction semantics, focused breach/report/reverse-media accessibility semantics, exact evidence-key fail-closed behavior, bounded CaseStore save behavior, graph/export redaction semantics, report-evidence semantics, async saved-case persistence, HUD status semantics and reverse-video picker semantics. A fresh external ADB `am force-stop`/relaunch and emulator-reboot smoke check on the current artifact restored the encrypted background result, confirmed `Identity Surface Map` and the “latest encrypted transient result is ready” state after both launches, and confirmed the production `MainActivity` resumed after both launches. The face, exposure, terminal projection, dangling-evidence, exact-only legacy provenance, exact resolver provenance, source-page-aware reverse-image candidate deduplication, graph relation normalization, graph entity evidence-ID normalization and deterministic truncation diagnostics, deterministic later-stage recovery telemetry, CaseStore security and WhatsMyName conversion-parity checks are JVM/no-network tested; face, post-processing, graph-stage, relationship-confidence, attack-path and exposure checkpoint reuse remain code-tested for exact request/plan/owner, input/model digest, size/shape, completed-stage, record-size fallback and TTL binding but were not independently process-kill validated on the emulator. Physical-device, broad accessibility and full-stage recovery gates remain open.
...
| Identity graph | **7/8** | Graph v2 adds semantic node kinds, typed relationships, evidence/contradiction IDs, node states, history fields, queries and schema versioning while preserving legacy case compatibility. Evidence-backed relationship endpoints now reuse one exact unique canonical graph entity where possible; ambiguous values keep an unresolved synthetic fallback. Entity and edge evidence IDs share trim/migration/deduplication semantics and a 256-ID bound. Duplicate edge detection canonicalizes relation keys by trim/case-insensitive comparison while preserving the first serialized spelling and unioning evidence IDs; this prevents formatting drift from creating parallel edges without inferring identity. Archive attributes are attached to historical archive-source nodes with typed relationships, and media candidates/clusters enrich the graph without identity claims. GraphML, declared-attribute GEXF and NetworkX-style node-link JSON serializers provide dependency-free interoperability with Gephi/Cytoscape/NetworkX-style workflows. Case graph exports now keep the projection separate from the read-only canonical scanner/plugin assertion ledger in a distinct CSV sidecar, with ShareSafe endpoint/evidence redaction. Read-only reconciliation diagnostics compare exact endpoint/relation/evidence sets, cap entity/edge evidence IDs at 256 per diagnostic and fail closed on dangling or truncated graph-entity provenance when a non-empty ledger is available; these diagnostics still do not replace the remaining parallel representations. |
...
Debug JVM unit tests          PASS — 839 tests / 137 result XML files / 0 failures / 0 errors / 0 skips
uiTest JVM unit tests         PASS — 839 tests / 137 result XML files / 0 failures / 0 errors / 0 skips
Android-test Kotlin compile  PASS — `compileUiTestAndroidTestKotlin`
Debug APK                     PASS — 119,492,799 bytes / SHA-256 5F3E9853B55071E9C46A3D76575F83530182F0600937AA3BDE0CC27994A8444F
uiTest APK                    PASS — 243,987,820 bytes / SHA-256 FF878EEBC04D372600A6DCDD1038724C260AD710928F41446F656912AB0E963B
...
Graph evidence-ledger audit    PASS — 14 focused JVM tests; dangling canonical, graph-edge and graph-entity evidence IDs fail closed when a non-empty ledger is supplied, graph-entity truncation emits deterministic bounded per-entity diagnostics and remains inconsistent, and legacy empty-ledger cases remain fail-soft
...
Face checkpoint                PASS — 5 codec/model-commitment JVM tests plus 6 encrypted resume/store tests covering bounded metadata, unsafe/non-finite/out-of-range rejection, identity/profile/model digest changes, model-race discard, request/plan/owner rebind, TTL, tamper, incomplete-stage and record-size rejection; no independent process-kill claim
CaseStore security             PASS — 8 JVM tests covering traversal/absolute/control/oversized ID rejection, canonical confinement, scoped temp cleanup, aggregate deletion and directory-sync failure; full write-durability hardening remains open
...
Graph relation normalization PASS — 12 EntityGraphBuilder + 6 GraphSchemaV2 + 14 GraphEvidenceReconciliation + 3 CaseEvidenceIdMigration JVM tests; case/whitespace variants merge without dropping provenance and entity evidence IDs are trimmed/migrated/deduplicated/bounded/reconciled
Recovery diagnostics            PASS — 5 ProfileInitialPassExecutor + 17 ScanCoordinatorRuntime JVM tests; exact direct-profile, post-processing and deterministic later-stage checkpoint reuse/rerun counts are sanitized, stage-allowlisted, count-bounded and rejected after terminal state
Visual QA                     NOT RUN in this validation session
```

The latest focused additions keep the pinned executable-rule health report separate from the 78 authored-provider report, publish sanitized direct-profile, post-processing and deterministic later-stage checkpoint reuse/rerun counts through the coordinator snapshot and active-scan HUD, normalize graph entity and relation evidence IDs by trim/migration/deduplication with a 256-ID bound, reconcile graph-entity provenance against the explicit evidence ledger with fail-closed truncation, normalize graph relation keys by trim/case-insensitive comparison without rewriting the first persisted relation spelling or dropping evidence IDs, persist a bounded face-consistency result only across a stable model commitment, and confine CaseStore deletion paths to safe case IDs and the canonical case directory.
