...
Not complete: true suspended pause/resume, sole coordinator ownership, universal in-flight payload coverage across every stage, a general coordinator-owned parser/frontier loop beyond the persisted plan commitment, migration of remaining custom resolver work to provider lifecycle events and full-stage recovery beyond the encrypted result path. Breach, deterministic post-processing, graph-stage, relationship-confidence and attack-path slices now have bounded encrypted reuse, but the worker still reruns earlier scan stages before reaching those boundaries. External force-stop/relaunch and emulator-reboot restoration of the result path now pass with a uiTest-only synthetic fixture.

The encrypted resume record stores the seed input, selected mode, deep-scan flag, per-scan strong-face-policy flag, the selected plan fingerprint plus bounded provider IDs for new requests, an allow-listed mode/stage-order plan summary, an allow-listed semantic stage ledger bound to the exact worker owner, bounded per-stage output counts, bounded breach-stage summaries, bounded post-processing analysis checkpoints, bounded graph-stage, relationship-confidence and attack-path checkpoints and public search/image payload summaries. New WorkManager rows receive only opaque request/generation UUID references. Prepared/current generations, plan metadata, stage metadata/output counts, breach/post-processing/graph/confidence/attack-path summaries and public search/image envelopes are still request checkpoints rather than a general parser/frontier state or universal in-flight stage payloads. The external smoke check validates only durable result restoration through the production UI with a synthetic fixture; it does not establish full-stage process-death/reboot recovery or physical-device acceptance.
...
Debug JVM unit tests          PASS — 796 tests / 132 result XML files / 0 failures / 0 errors / 0 skips
uiTest JVM unit tests         PASS — 796 tests / 132 result XML files / 0 failures / 0 errors / 0 skips
Android-test Kotlin compile  PASS — `compileUiTestAndroidTestKotlin`
Debug APK                     PASS — 115,614,184 bytes / SHA-256 EAFD3FB0DE9E18767B3FB80DB69E41D407BB88D514FD83630DE433874804422F
uiTest APK                    PASS — 243,128,761 bytes / SHA-256 480C1C6A31C4C6C8D72CB94A68355011F3074B474262E534B7C0F4F629B049B8
