...
- **Last validated implementation commit:** `02fb506` (`feat: persist scan stage checkpoints and provider health`)
- **Last validation date:** **2026-08-25**
- **Current-session validation of the implementation plus the preserved pre-existing uiTest fixture edit:** provider and pinned-catalog audits passed; both debug and uiTest JVM suites passed 644 tests across 110 suites each; debug/uiTest APKs assembled; Android-test Kotlin compiled; lint completed with zero errors; and a clean rerun of the complete connected uiTest suite passed 40/40 on the API 36 `medium_phone` emulator, including the 8 WorkManager pause/resume tests. One earlier emulator run had a single transient UI timeout; the isolated test and immediate full rerun passed. Physical-device gates remain open.
...
| Testing/device validation | **3/5** | Provider audit (78 definitions), pinned WhatsMyName integrity audit (716 records / 644 executable rules), both debug and uiTest JVM suites (644 tests across 110 suites each, 0 failures/errors/skips), debug/uiTest APK assembly, Android-test Kotlin compilation, zero-error lint and a clean rerun of the complete connected uiTest suite (40 tests, including 8 WorkManager pause/resume tests) pass on the exact implementation commit above using the API 36 `medium_phone` emulator. Physical Samsung/Pixel/lower-memory, battery, external process-death/reboot and complete accessibility gates remain incomplete. |
| **Total** | **76/100** | Declarative provider extraction, fail-closed redirect/WebView boundaries, persisted provider-health assessment, a configured-depth bounded frontier drain, exact-owner background pause/resume, encrypted semantic stage-boundary metadata/events, bounded graph-provenance redaction, encrypted media persistence/history, historical attribute extraction and off-main UI persistence seams are now covered by current-tree tests and builds. This does not establish sole coordinator ownership, persisted stage outputs/general plan, process-kill/reboot acceptance, registry-wide live validation, representative empirical calibration, connected-device acceptance or release hardening. |
...
Not complete: 1,000+ useful reviewed providers, registry-wide live present/missing/soft-404/redirect/challenge validation (including representative WhatsMyName rules), surfaced catalog-wide health dashboards/maintenance workflows, and migration of every mature custom resolver to the declarative runtime.
...
The encrypted resume record stores the seed input, selected mode, deep-scan flag, per-scan strong-face-policy flag, the selected plan fingerprint plus bounded provider IDs for new requests, and an allow-listed semantic stage ledger bound to the exact worker owner. New WorkManager rows receive only opaque request/generation UUID references. Prepared/current generations and stage metadata are still request checkpoints rather than a general scan plan/parser/frontier state or persisted stage outputs, and deterministic crash-boundary/instrumentation coverage does not by itself constitute process-death or reboot acceptance.
...
Not complete: one coordinator-owned general plan covering every scan stage, checkpointed search/image/frontier outputs, cross-process file locking, eager expired-file pruning and external process-kill/reboot acceptance. The semantic ledger records breach/AI boundaries but does not persist their in-flight outputs or establish universal pause/resume semantics.
...
Not complete: graph-only truth across every subsystem, complete evidence IDs on legacy edges, richer graph-history review, and full archive/image/breach population at contract depth.
...
Implemented in this tranche: bounded cross-case/cross-scan fingerprint grouping with per-case observations, retained source-page/provider/retrieval/hash provenance, case-local handling when fingerprints are absent, and accessible saved-case review wording. Not complete: verified-account correlation, broader change-diff workflows and independent visual acceptance. Face similarity remains a separate supporting signal.
...
Not complete: equivalent structured extraction across every archive/provider source, universal archive timestamp propagation and complete change-diff UX.
...
Not complete: production timeline, corrections directly on every live Evidence card, mature tablet/landscape adaptation, localization/RTL and complete accessibility validation of the new pause/media surfaces.
...
Debug JVM unit tests          PASS — 644 tests / 110 suites / 0 failures / 0 errors / 0 skips
uiTest JVM unit tests         PASS — 644 tests / 110 suites / 0 failures / 0 errors / 0 skips
Android-test Kotlin compile  PASS — `compileUiTestAndroidTestKotlin`
Debug APK                     PASS — 118,780,131 bytes / SHA-256 82DCDCAF23E11120C273F411F1A3AEF12802536F3BADEDA9F70237F9D84963C6
uiTest APK                    PASS — 243,319,921 bytes / SHA-256 31E646AE024BE15DC3A47F54470ED36928B0ADB3BEC88AB9EC6E3392B8752AB9
...
Connected uiTest suite         PASS — 40 tests / 0 failures on API 36 `medium_phone` emulator (8 WorkManager pause/resume tests included; clean full rerun after one transient UI timeout)
Visual QA                     NOT RUN in this validation session
```

The deterministic JVM/build gate was run with the declarative extractor, provider-health buckets, redirect-policy, WebView-policy, frontier-depth, AI-key, encrypted-plan-fingerprint, encrypted stage-ledger owner binding, remote graph-provenance, encrypted result-store bounds, pause/resume lifecycle, media-history, face-calibration attestation, off-main persistence and historical attribute tests included. The complete connected uiTest suite then ran on the API 36 `medium_phone` emulator; its 40 tests covered consent/UI smoke flows, encrypted case/case-comparison behavior, timeline/history, saved-case AI, WorkManager ownership and the delayed exact-owner pause/resume lifecycle. One initial full run had a single UI timeout; the isolated test and immediate clean full rerun passed. The uiTest-only receiver edit that was already present in the worktree remains uncommitted; the APK hashes above therefore describe current-worktree artifacts, not a clean checkout of the implementation commit. No physical-device, process-death/reboot, accessibility or battery/thermal claim is made here.
...
2. Migrate remaining mature custom resolvers to the v2 runtime and surface the persisted provider-health reports in maintenance/diagnostic UI; current buckets are not registry-wide live validation.
3. Extend the configured-depth frontier into a coordinator-owned general plan with persisted stage outputs/frontier state and surfaced diagnostics for every stage, then run external process-kill/relaunch and reboot recovery validation against the production lifecycle. Background WorkManager pause/resume and semantic stage metadata are exact-owner bounded, but they are not yet one general coordinator pause/resume contract.
...
9. Extend historical attribute extraction beyond directly re-fetched Wayback HTML and finish change-diff UX across archive/provider sources.
10. Build a production AI evaluation corpus and corrected-graph-native input path.
11. Complete live Evidence correction UX and broad provider-specific remediation/recheck workflows; graph export/share actions now exist but still need broader acceptance review.
