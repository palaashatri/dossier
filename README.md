...
`45520a1` adds fail-closed frontier recovery for unreadable persisted pivot state and routes deep-research website traversal through the bounded declarative provider runtime with opaque host IDs, lifecycle events, redirect/auth/challenge classification, response limits and the existing six-page bound. `5f5ba8d` hardens calibration artifacts so configured contradiction weights cannot disable negative evidence. The full Android artifact gate remains recorded against the prior `8cbef92` source tree until this freeze point is rebuilt.
...
Debug JVM unit tests                PASS — 856 tests / 139 result XML files / 0 failures, errors, or skips
uiTest JVM unit tests               PASS — 856 tests / 139 result XML files / 0 failures, errors, or skips
Android-test Kotlin compilation    PASS — `compileUiTestAndroidTestKotlin`
Debug APK assembly                  PASS — 119,510,683 bytes
Debug APK SHA-256                   F74F40BA323ED34EB8663A9C996C176BA379D8EC41156BC11B9BAD5FD9A25FA9
uiTest APK assembly                 PASS — 243,992,586 bytes
uiTest APK SHA-256                  DE88A3E1481F8213B575D8F53989595063D8A7E190AC6230529C6DBBE92E0060
Android-test APK                    PASS — 1,027,632 bytes
Android-test APK SHA-256             CBFB4697EDC1C1A466C097B6D39715C3F94D03FB02EEB917FAF6285A2AFED1C7
...
The APK, lint, connected-uiTest and external recovery evidence in this block is the prior `8cbef92` artifact gate; the current `45520a1` source has not been rebuilt through those Android gates in this freeze turn.

The complete connected uiTest suite ran on the API 36 `medium_phone` emulator, including the provider-health panel, 8 WorkManager pause/resume tests, encrypted Activity-recreation recovery, live finding/profile/media correction semantics, exact evidence-key fail-closed behavior, focused breach/report/reverse-media accessibility semantics, graph/evidence reconciliation status semantics, bounded CaseStore save behavior, async saved-case persistence, graph/export redaction semantics, HUD status semantics and reverse-video picker semantics. The current tree also includes direct-profile and deterministic later-stage recovery diagnostics, graph entity-provenance, face-checkpoint codec/store/model-race and CaseStore path/deletion security regressions in the JVM suites. A fresh ADB smoke check seeded the production encrypted result store through the uiTest-only fixture, then verified `Identity Surface Map` and the “latest encrypted transient result is ready” state after an external `am force-stop`/relaunch and after an emulator reboot, with the production `MainActivity` resumed after both launches. The uiTest APK was built with a pre-existing uncommitted fixture-only edit that remains outside the implementation commits; its hash is therefore a current-worktree artifact. Physical Samsung/Pixel/lower-memory devices, broad TalkBack/switch/keyboard, battery/thermal behavior, and full-stage process recovery remain unvalidated. Face, post-processing, graph-stage, relationship-confidence, attack-path and exposure checkpoint reuse is code-tested for exact binding/digest/shape/TTL/completed-stage behavior (including model-change rejection and graph record-size fallback), and deterministic later-stage reuse/rerun telemetry is projection-tested, but no checkpoint has been independently process-kill validated on the emulator. Terminal projection rejection, graph entity/edge dangling-reference checks, exact-only legacy provenance, exact resolver support/contradiction provenance, source-page-aware reverse-image deduplication and WhatsMyName conversion parity are JVM/no-network tested; no claim of broad visual acceptance is made.
...
- The username-discovery flow surfaces separate persisted reports for the authored catalog and the pinned executable WhatsMyName rules, with bounded nonhealthy previews and explicit wording that catalog membership, HTTP 200 or search hits are not live validation; internally inconsistent counters and future validation timestamps are shown as unavailable rather than healthy.
...
- Scanner/plugin relationship assertions with the same normalized endpoints and a trim/case-insensitive relation key are merged deterministically while unioning their evidence IDs and retaining the first nonblank description, so one producer cannot erase another producer's provenance.
...
- After a completed direct-profile pass, the coordinator publishes sanitized request-scoped recovery diagnostics showing exact checkpoint reuse versus rerun counts; the event carries no candidate, URL or provider payload.
...
- Case-level graph reconciliation can optionally audit canonical relationship, graph-edge and graph-entity evidence IDs against the persisted evidence ledger, marking unresolved or truncated provenance as inconsistent instead of treating matching endpoint/relation projections as sufficient; legacy cases with no evidence ledger remain explicitly fail-soft.
...
Optional local cross-photo face support uses pinned YuNet/SFace models with exact size/SHA-256 verification, deterministic preprocessing, five-landmark alignment, ambiguity/quality rejection and cosine scoring. Each comparison now retains bounded structured backend/model-hash/pipeline, calibration-state and selfie/profile-quality provenance, including explicit reference-policy, measured, imported, fallback and not-run states. Background requests may retain an encrypted bounded face-consistency checkpoint bound to the identity/profile/model digest; a model commitment change discards the checkpoint rather than reusing a score under a different backend. Face similarity remains supporting evidence; release thresholds are not advertised as measured identity probabilities until a representative benchmark exists.
...
- CaseStore case IDs are restricted to bounded ASCII-safe identifiers and every encrypted, legacy, backup and temporary path is canonically confined to the case directory; aggregate delete/clear attempts all scoped files, includes temporary files, fsyncs the directory and fails closed on unsafe entries, failed deletion or failed directory sync. This does not claim full write-durability hardening for every legacy synchronous helper.
