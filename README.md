...
Debug JVM unit tests                PASS — 796 tests / 132 result XML files / 0 failures, errors, or skips
uiTest JVM unit tests               PASS — 796 tests / 132 result XML files / 0 failures, errors, or skips
Android-test Kotlin compilation    PASS — `compileUiTestAndroidTestKotlin`
Debug APK assembly                  PASS — 115,614,184 bytes
Debug APK SHA-256                   EAFD3FB0DE9E18767B3FB80DB69E41D407BB88D514FD83630DE433874804422F
uiTest APK assembly                 PASS — 243,128,761 bytes
uiTest APK SHA-256                  480C1C6A31C4C6C8D72CB94A68355011F3074B474262E534B7C0F4F629B049B8
...
Recovery now covers stable initial direct-profile outcomes, the configured-depth pivot frontier, an exact-owner background pause/resume lifecycle, an encrypted mode/provider-plan/stage-order commitment, semantic stage-boundary metadata, bounded completed-stage output counts, encrypted public-search/public-image retry payloads, encrypted breach summaries, encrypted deterministic post-processing output and an optional encrypted graph-stage output. The payload envelopes are request/plan/stage-bound, bounded, TTL-limited, tamper-checked and cleared with request-scoped tombstones; verified profile outcomes cannot enter these public-discovery caches. A coordinator-owned parser/frontier plan, universal in-flight payload persistence, sole coordinator ownership and full-stage resume semantics remain incomplete: a recreated worker still reruns earlier stages before it can reuse a later deterministic output. Pivot diagnostics are persisted and the background pause state is surfaced, while some mature custom resolvers still need migration to the declarative execution path. An ADB-driven force-stop/relaunch and emulator-reboot smoke check restored the encrypted background result through the production UI using a uiTest-only synthetic fixture; it does not establish full-stage worker recovery or physical-device acceptance.
